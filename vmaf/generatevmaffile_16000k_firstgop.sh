...
#   判断的时候记得进行字符串拼接
  ...
  then
    ...
    # 第一步：生成file
     /usr/bin/time ffmpeg -i "$dis_path$f264gop" -i "$ref_path$f" -lavfi "[0:v][1:v]libvmaf=psnr=1:log_path=/home/wudi/desktop/dataset_operation/vmaf/logfile_firstgop/${fname}.txt" -f null -
