...
#   判断的时候记得进行字符串拼接
  ...
  then
    ...
    # 第一步：生成file
     /usr/bin/time ffmpeg -i "$output_path$f264" -i "$path$f" -lavfi "[0:v][1:v]libvmaf=psnr=1:n_subsample=30:log_path=/home/wudi/desktop/dataset_operation/vmaf/logfile_sample30/${f}.txt" -f null -
