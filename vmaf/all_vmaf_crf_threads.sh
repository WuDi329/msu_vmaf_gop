...
for i in $(seq 0 51); do
    ...
    then
        ...
        # ffmpeg psnr score
        /usr/bin/time ffmpeg -nostats -i "$output_path$f" -i "$path$f" -lavfi "[0:v][1:v]libvmaf=psnr=1:n_threads=8" -loglevel info -f null - 2> temp_output.txt
        ...
        echo "$avg_vmaf   $exe_time   $f" >> /home/wudi/desktop/vmaf_gop_test/crf_vmaf_threads3/${i}.txt
