...
for i in $(seq 19 19); do
    ...
    then
        ...
        # ffmpeg psnr score
        /usr/bin/time ffmpeg -nostats -i "$output_path$f" -i "$path$f" -lavfi "[0:v][1:v]libvmaf=psnr=1" -loglevel info -f null - 2> temp_output.txt
        ...
        echo "$avg_vmaf   $exe_time   $f" >> /home/wudi/desktop/vmaf_gop_test/crf_vmaf_3/${i}.txt
