...
#   判断的时候记得进行字符串拼接
  ...
  then
    ...
    # ffmpeg psnr score
    /usr/bin/time ffmpeg -nostats -i "$output_path$f264" -i "$path$f" -lavfi "[0:v][1:v]libvmaf=psnr=1:n_subsample=50" -loglevel info -f null - 2> vmaf_output_sample50.txt
    avg_vmaf=$(awk '{a[NR]=$0}END{print a[NR-2]}' vmaf_output_sample50.txt | awk '{print $6}' )
    avg_vmaf=${avg_vmaf}

    exe_time=$(awk '{a[NR]=$0}END{print a[NR-1]}' vmaf_output_sample50.txt | awk '{print $3}')
    exe_time=${exe_time%elapsed}

    echo "$avg_vmaf   $exe_time   $f" >> vmaf_result_sample50.txt
